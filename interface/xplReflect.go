...
//    log.Printf("Using csr file: %s\n", csrFilnam)
    ...
    e := reflect.ValueOf(&testStructA).Elem()
	...
	fmt.Println("\n *** reflect interface ***\n")
	PrintInfo(test)
    elTyp := reflect.TypeOf(test)
    elVal := reflect.ValueOf(test)
	fmt.Printf("el value: %v elTyp: %v kind: %s\n", elTyp, elVal, elVal.Kind())
